...
class BathScene: SKScene, SKPhysicsContactDelegate {
    ...
    var stafood = "" {
        didSet {
            if !stafood.isEmpty {
                ...
                foodNode.physicsBody?.categoryBitMask = ColliderType1.bath
                foodNode.physicsBody?.contactTestBitMask = ColliderType1.pet
                foodNode.name = "Food"
    ...
    override func didMove(to view: SKView) {
        ...
        do {
            ...
            for user in users {
                print("\(user[self.staSceneBath])")
                if user[self.staSceneBath] == "bathscene"{
                    background = SKSpriteNode(imageNamed: "bathscene")
                    print("bathScene")
                }
                else{
                    let long = user[self.staSceneBath]
                    background = SKSpriteNode(imageNamed: long)
                    print("ฉากอื่น \(long)")
                }
                print("end \(user[self.staSceneBath])")
            }
        } catch {
            print("what error")
            print(error)
    ...
    func buildTest() {
        ...
        let typeAni = loadPet()
        let ranSet = Int(arc4random_uniform(4))
        if typeAni == 2
        {
            if (ranSet%4) == 0{
        ...
        test.physicsBody?.categoryBitMask = ColliderType1.pet
        test.physicsBody?.contactTestBitMask = ColliderType1.bath
        test.name = "Pet"
